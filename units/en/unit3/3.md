...
- Master DPO training with SmolLM3 using real preference datasets
- Understand how to configure DPO hyperparameters for optimal alignment
- Use Hugging Face Jobs for scalable DPO training in the cloud
- Evaluate alignment quality using preference benchmarks
...
def main():
    ...
    # Load SmolLM3-3B-Instruct model (pre-trained with SFT)
    model_name = "HuggingFaceTB/SmolLM3-3B-Instruct"
...
Ready to submit your aligned model to the leaderboard? Continue to the [submission page](6) where you'll:
...
🎯 **What You've Learned**:
- **DPO Theory**: How DPO directly optimizes preference alignment without reward models
- **Practical Implementation**: Using TRL and HF Jobs for scalable DPO training
- **Hyperparameter Tuning**: Balancing beta, learning rate, and training steps for optimal alignment
- **Cloud Training**: Leveraging HF Jobs for resource-intensive preference alignment

🎯 **DPO Best Practices**:
- Start with instruction-tuned models (not base models)
- Use lower learning rates than SFT (5e-7 typical)
- Beta parameter controls alignment strength (0.1 is a good starting point)
- Quality preference data is crucial for good results

🎯 **Next Steps**:
- Experiment with different preference datasets
- Try various beta values to balance alignment vs creativity  
- Explore multi-objective DPO for complex alignment goals
- Compare DPO results with RLHF approaches
