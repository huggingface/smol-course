...
!pip install transformers>=4.36.0 trl>=0.7.0 datasets>=2.14.0 torch
...
2. **Long Context**: Create a very long conversation and test the 128k context window
...
training_config = SFTConfig(
    ...
    # Experiment tracking
    report_to=["tensorboard"],  # or ["wandb"] if using W&B
    run_name=f"{new_model_name}-{wandb.run.id if 'wandb' in locals() else 'local'}",
)
...
**🎯 Objective**: Learn to use TRL's command-line interface for scalable, production-ready fine-tuning workflows.
