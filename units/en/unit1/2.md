...
Understanding the difference between base and instruct models is crucial for effective fine-tuning:
...
**Instruct Model (`SmolLM3-3B`)**: Fine-tuned to follow instructions and engage in conversations. If you ask "What's the weather like?", it understands this as a question requiring a helpful response.

### The Transformation Process

The journey from base to instruct model involves:

1. **Chat Template Application**: Converting raw conversations into a structured format
2. **Supervised Fine-Tuning**: Training the model to generate appropriate responses
3. **Preference Alignment**: Optional refining responses to match human preferences
...
SmolLM3 uses a sophisticated chat template that handles multiple conversation types. Let's examine how it works:

### ChatML Format Structure

SmolLM3 uses the ChatML format with special tokens that clearly delineate different parts of the conversation:
...
SmolLM3's unique feature is its support for dual-mode reasoning through special formatting:
...
This dual-mode capability allows SmolLM3 to show its reasoning process when needed, making it perfect for educational applications and complex problem-solving.

## Working with SmolLM3 Chat Templates in Code

The `transformers` library automatically handles chat template formatting through the tokenizer. This means you only need to structure your messages correctly, and the library takes care of the special token formatting.

### Basic Usage

Here's how to work with SmolLM3's chat template:
...
System messages are crucial for controlling SmolLM3's behavior. They act as persistent instructions that influence all subsequent interactions:
...
- **Use examples**: Show the desired response style when possible

## Multi-Turn Conversations

SmolLM3 excels at maintaining context across multiple conversation turns. Each message builds upon the previous context:
...
SmolLM3 can handle up to **128k tokens** of context, which means very long conversations or documents. However, for efficiency:

- **Keep relevant context**: Include only necessary conversation history
- **Summarize when needed**: For very long conversations, consider summarizing earlier parts
- **Use system messages**: Set persistent context that doesn't need repetition
...
- **For inference**: Always use `add_generation_prompt=True` when you want the model to generate a response
...
The Transformers library provides automated pipelines that handle chat templates seamlessly, making it easy to use chat models without manual template management.

### Text Generation Pipeline with Chat
...
The text generation pipeline automatically:
- Applies the correct chat template for the model
...
### Fine-Tuning with Chat Templates

You can fine-tune SmolLM3 using either Python code or the command-line interface:
...
# Train with SFTTrainer (handles chat templates automatically)
trainer = SFTTrainer(
    model=model,
    tokenizer=tokenizer,
...
Both approaches automatically handle chat template processing during fine-tuning.

## Chat Templates in Training: Best Practices

Using chat templates correctly during training is crucial for model performance. Here are the key considerations and best practices:
...
chat_dataset = load_dataset("HuggingFaceTB/smoltalk2", "SFT", split="train[:1000]")
...
def format_for_training(examples):
    ...
    for messages in examples["messages"]:
        ...
        formatted_text = tokenizer.apply_chat_template(
            messages,
            tokenize=False,
            add_generation_prompt=False,  # Important: False for training!
            add_special_tokens=False  # Template already includes needed tokens
        )
...
# Apply formatting
training_dataset = chat_dataset.map(format_for_training, batched=True)
...
When training on multiple datasets with different formats:

```python
def standardize_dataset_format(examples, dataset_type):
    """Convert different dataset formats to standard chat format"""
    messages_list = []
    
    if dataset_type == "instruction_following":
...
instruction_data = load_dataset("some/instruction_dataset")
qa_data = load_dataset("some/qa_dataset")
conversation_data = load_dataset("HuggingFaceTB/smoltalk2", "SFT")

# Standardize formats
instruction_formatted = instruction_data.map(
    lambda x: standardize_dataset_format(x, "instruction_following"),
    batched=True
)

qa_formatted = qa_data.map(
    lambda x: standardize_dataset_format(x, "qa_pairs"),
    batched=True
)
...
4. **Context Management**: Leverage SmolLM3's 128k context window efficiently by managing conversation history
...
- **Error Handling**: Implement proper validation for message formats and role sequences
...
⏭️ [Next: Supervised Fine-Tuning](./3.md)
...
- [Discord Server](https://discord.gg/huggingface) - Real-time help and community interaction
